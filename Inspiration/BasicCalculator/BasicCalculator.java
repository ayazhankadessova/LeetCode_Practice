...
class BasicCalculator {
    ...
    // Basic Setup:
    // 1. Create a char array to store operands and operators, everything from
    // string value.
    ...
    // Case '(' -> need to find res inside the brackets & add it to res w the
    // operator-> call dfs inside the bracket:
    // index++;
    // sum += oper * dfs res
    // Case ')' -> brackets res ended:
    // break;
    // Case '+' / '-' -> set operator to +1/-1;
    // Case digit -> can have multiple digit numbers, so iterate while char is not
    // digit & add to StringBuilder
    // * don't forget index-- here bc we stop on smth which isn't a digit, we need
