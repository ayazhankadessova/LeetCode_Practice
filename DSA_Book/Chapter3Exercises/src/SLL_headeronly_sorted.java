...
4. add: check if contains, if not, create p 
    * **To keep sorted** , check if it is more than header.next. 
...
6. remove: check if contains. Do tracing node and current node. When found node to remove -> prev.next = current.next
...
public class SLL_headeronly_sorted<T extends Comparable<T>> {
    ...
    /*
     * Add preserving ascending order
     */
    boolean add(T data) {
        if (contains(data)) {
            return false;
    ...
    /*
     * Set a node which will trace the previous elem and when we find an item to
     * delete make the tracer point to deleted's next node.
     */
